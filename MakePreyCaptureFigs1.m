...
load('multAni_test_071420_COMPILED_c.mat')
...
d_mnEyeAll=[];allDLChead=[];mnEyeAll=[]; mouseSpAll=[]; allAppT=[];
for vid=1:length(approachEpochs)
    nframe=min(length(dthetaR{vid}),length(dthetaL{vid}));
    ...
    approaches=approachEpochs{vid}(1:nframe);
